...
**Pre-write anti-hypothesis audit.** Before rendering `triage.md`, scan the planned `Notes` and `Code findings` sections for root-cause-shaped statements (e.g., "the root cause is…", "this happens because…", "appears to be a timing issue", "may be caused by…", "the bug manifests because…"). If any are present, rewrite them as factual observations (what was grepped, what exists, what was changed when) — or remove them. Cause-and-effect synthesis belongs to `/gutenberg-fix`, never here.
